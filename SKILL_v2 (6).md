...
2. From the response, extract:
   - The `customfield_XXXXX` key holding Manual Steps. **Manual Steps in Xray Server/DC is typically not writable via a plain field value in this GET response** — it may show as a summary/count rather than the full step array. If the GET response does not show the full Action/Data/Expected Result structure per step, do not guess the write format. Instead call `GET {base}/rest/raven/1.0/api/test/FSD-12945/step` (Xray's dedicated steps endpoint — resource is `test`, not `testcase`) to see the real structure, and use `POST {base}/rest/raven/1.0/api/test/{TEST-KEY}/step` to add each step individually after the issue is created — one call per step, in order — rather than trying to write all 4 steps as part of the initial Create Issue payload. **Phase 0 verification of this endpoint is via the GET call only — never call the POST step-adding endpoint against FSD-12945 or any other existing issue. The POST endpoint is only ever called against a newly-created test case in Phase 2, never as a "test" or "check" during Phase 0.**
...
- **Create issue:** `POST {base}/rest/api/2/issue` with `"project": {"key": "FSD"}`, `"issuetype": {"name": "Test"}`, `"priority": {"name": "Normal"}`, `"summary": "<Title>"`, `"description": "<Description>"`, the Test Level custom field (`Regression`), the Test Case Type custom field (`Functional`), and Assignee (identifier from Phase 0 `/myself`). Jira **auto-assigns** the issue key (e.g. `FSD-1042`) in the response — never construct, guess, or hardcode an issue key yourself; always read it from the create-response.
- **Add Manual Steps:** if Phase 0 found Manual Steps writable directly as part of the issue payload, include it in Create Issue above. Otherwise, immediately after Create Issue returns the new key, call `POST {base}/rest/raven/1.0/api/test/{TEST-KEY}/step` (resource is `test`, not `testcase`) once per step (5 calls, in order 1→5), each with that step's Action/Data/Expected Result values. If any of the 5 calls fails (e.g. 404, 400, or any non-success response), do not continue adding remaining steps — immediately roll back per the all-or-nothing rule in Phase 2.
...
- **Assign to folder:**
  1. `GET {base}/rest/raven/1.0/api/testrepository/FSD/folders` — resolve the numeric `folderId` (root folder is `-1`) by walking the returned tree to find the folder matching `Test Repository/The INTcredibles/API-Manual/SessionRefreshContext`. Note: this endpoint takes the **project key** (`FSD`) directly, not a separate numeric project ID.
  2. `PUT {base}/rest/raven/1.0/api/testrepository/FSD/folders/{folderId}/tests` with body `{"add": ["<newly created issue key>"]}`.
...
For each confirmed row, in order — **all-or-nothing per row, no partial test cases left behind:**
...
3. Add all 4 Manual Steps (inline or via separate calls, per Phase 0's finding). **If any step call fails, immediately attempt `DELETE {base}/rest/api/2/issue/{issue key}` to remove the incomplete issue.**
   - **If the DELETE succeeds:** stop processing further rows and report `<<CHECK: step-add failed for <row>, issue <key> was created then rolled back — do not retry until the underlying API issue is fixed>>`.
   - **If the DELETE itself also fails** (e.g. permissions, or any non-success response): do NOT claim the issue was rolled back. Stop immediately and report `<<CHECK: step-add failed for <row>, issue <key> was created but could NOT be automatically deleted (delete call failed) — this issue is left in Trackspace incomplete and needs manual deletion or fixing by the user>>`.
   - Either way, do not leave an issue in Trackspace with a Summary/Description but missing or partial steps without it being explicitly flagged to the user.
4. Only if all 4 steps succeeded, resolve the folder's numeric `folderId` and call Assign to Folder. If this fails, treat it the same as a step failure: roll back (delete the issue) rather than leaving it unfiled with steps but no folder placement.
5. Report back per row only once every part (issue + all steps + folder) succeeded: `<Title> → created as <issue key>, filed under <folder path>` (or the rollback/error reason if any part failed).
6. After all rows are processed, print a summary: `Total: <N> rows processed — <X> fully created, <Y> skipped as duplicate, <Z> rolled back due to partial failure, <W> flagged with <<CHECK>>.`
...
- Never guess the Manual Steps custom field ID or a folder ID — resolve them via the documented API calls or fetched real-issue payload, never from memory or assumption.
...
- Never guess whether Manual Steps is writable inline vs. via separate step-add calls — confirm from Phase 0's actual GET response before choosing the approach.
- Always print the end-of-run summary tally, even if some rows were skipped or flagged.
- **A test case is never considered created unless the issue, all 4 Manual Steps, and folder placement all succeed together.** If any part fails partway through, attempt to delete the partially-created issue immediately rather than leaving a Summary/Description-only (or steps-missing) issue sitting in Trackspace. Never report a row as "created" unless every part actually succeeded, and never claim a rollback succeeded unless the DELETE call itself actually returned success.
