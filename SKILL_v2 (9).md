...
4. Only if all 4 steps succeeded, call Assign to Folder using the fixed `folderId` (44528) from the Trackspace System Details section above. If this fails, treat it the same as a step failure: roll back (delete the issue) rather than leaving it unfiled with steps but no folder placement.
...
- Never guess the Manual Steps custom field ID — resolve it via the documented API calls or fetched real-issue payload, never from memory or assumption. The target folder's `folderId` (44528) is a fixed, user-confirmed value — do not attempt to re-derive or second-guess it.
