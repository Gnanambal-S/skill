...
## Instructions
1. **Sheet name is given in the prompt for each run** — use exactly what's specified there, do not assume or reuse a previous run's sheet name.
...
- **Duplicate check (before creating) — exact match on Summary AND Description required, not Title alone:** Run `GET {base}/rest/api/2/search?jql=project=FSD AND summary~"<exact Title>"` to get *candidate* issues only — this JQL operator does fuzzy/contains text search, not exact matching, so it will often return unrelated issues that merely share common words (e.g., "country", "tenant"). **Title alone is not sufficient to declare a duplicate** — two genuinely different rows (e.g., same Context but different `jwt token` claims) can produce an identical Title, since Title only reflects Context/Refresh-context parameter names, not `jwt token` or `Refresh token` content. For each candidate returned by the search, fetch its full issue and compare **both** its `summary` AND its `description` fields, character-for-character, against the row's intended new Title and Description. Only skip creation if **both** fields are an exact, full-string match to an existing issue. If the Title matches but the Description differs, this is NOT a duplicate — proceed to create it, even though the Title looks the same as an existing issue.
- **Assign to folder:**
...
1. Run the duplicate check (fuzzy search for candidates, then exact Summary AND Description comparison — see "Duplicate check" above). If both fields exactly match an existing issue, skip and flag it — do not create.
...
- Always run the duplicate-title check before creating a new issue; skip and flag instead of creating a likely duplicate. **The fuzzy JQL search result alone is never sufficient to declare a duplicate, and Title alone is never sufficient either — two different rows can share an identical Title while having different Description content (Title only reflects parameter names, not full values). Always verify an exact match on BOTH Summary and Description before skipping creation.** A Title match with a differing Description is not a duplicate and must not block creation.
