...
- **Type:** Test
- **Priority:** Normal
- **Title:** `JWT & Refresh - Context: <param names from Context> - Refresh[Before|After]Expiry: <param names from Refresh context cell, or "with no context" if Nil>` — parameter names only, never actual values (values may be long/sensitive, e.g. tokens).
...
2. From the response, extract:
   - The `customfield_XXXXX` key holding Manual Steps. **Manual Steps in Xray Server/DC is typically not writable via a plain field value in this GET response** — it may show as a summary/count rather than the full step array. If the GET response does not show the full Action/Data/Expected Result structure per step, do not guess the write format. Instead call `GET {base}/rest/raven/1.0/api/testrun/FSD-12945/test` or `GET {base}/rest/raven/1.0/api/test/FSD-12945/steps` (Xray's dedicated steps endpoint) to see the real structure, and use `POST {base}/rest/raven/1.0/api/testcase/{TEST-KEY}/step` to add each step individually after the issue is created — one call per step, in order — rather than trying to write all 5 steps as part of the initial Create Issue payload.
...
- **Add Manual Steps:** if Phase 0 found Manual Steps writable directly as part of the issue payload, include it in Create Issue above. Otherwise, immediately after Create Issue returns the new key, call the dedicated Xray step-adding endpoint identified in Phase 0 once per step (5 calls, in order 1→5), each with that step's Action/Data/Expected Result values.
...
3. Resolve the folder's numeric `folderId` (once per run, reuse for all rows) and call Assign to Folder with the captured issue key.
4. Report back per row: `<Title> → created as <issue key>, filed under <folder path>` (or the skip/error reason).
5. After all rows are processed, print a summary: `Total: <N> rows processed — <X> created, <Y> skipped as duplicate, <Z> flagged with <<CHECK>>.`
